...
const widget = String.raw`(() => {
  ...
  // Escape any string before placing it inside HTML.
  const esc = (v) => String(v == null ? "" : v).replace(/[&<>"'\/]/g, (c) => ({ "&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;","/":"&#47;" }[c]));
  // Only allow safe CSS color tokens (hex, rgb/rgba, or a small keyword set); anything else falls back to a neutral default.
  const safeColor = (v) => {
