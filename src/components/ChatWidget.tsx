...
import { getPublicChatbotSettings, captureChatLead } from "@/utils/chatbot.functions";
...
export function ChatWidget() {
  ...
  useEffect(() => {
    getPublicChatbotSettings().then((c) => setConfig(c as Config)).catch(() => {});
  }, []);
  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      // Prompt for lead after a few exchanges
  ...
  return (
    <>
      ...
      {/* Panel */}
      {open && (
        <div className="fixed bottom-24 right-5 z-[100] flex h-[560px] w-[calc(100vw-2.5rem)] max-w-[380px] flex-col overflow-hidden rounded-2xl border border-black/10 bg-white/90 shadow-2xl backdrop-blur-xl animate-in fade-in slide-in-from-bottom-4">
